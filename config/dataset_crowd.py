...
def train_dataset(seed=3):
    image_folder = '/data/guanmandan/coco2014/trainval2014'
    root = '/data/guanmandan/coco2014/train2014'  #train的数据集
    source = '/data/guanmandan/coco2014/cocosplit/datasplit/trainvalno5k.json'   #train的标注文件
    batch_per_gpu = 2
    short_size = 800
    max_size = 1400
    ...
    while file_idx < nr_files:
        ...
        for i in range(batch_per_gpu):
            ...
            image_path = os.path.join(image_folder,record['images']['file_name'] + '.jpg')
        ...
        batch_images = np.zeros(
            ...
            dtype=np.float32)
        batch_gts = np.zeros(
            (batch_per_gpu, max_boxes_of_image, nr_box_dim),
            dtype=np.float32)
