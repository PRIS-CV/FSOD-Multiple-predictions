...
class FastRCNNOutputs(object):
    ...
    def diff_deltas_loss(self):
        return -F.l1_loss(self.pred_proposal_deltas[0], self.pred_proposal_deltas[1])*0.001
    
    def diff_logits_loss(self):
        return -F.l1_loss(self.pred_class_logits[0], self.pred_class_logits[1])*0.001
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Add !!!!!!!!!!!!!!!!!!!!!!!!!
        self.cls_score_0 = Linear(input_size, num_classes + 1)
        self.cls_score_1 = Linear(input_size, num_classes + 1)
        num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        box_dim = len(box2box_transform.weights)
        self.bbox_pred_0 = Linear(input_size, num_bbox_reg_classes * box_dim)
        ...
        self.cls_scores = [self.cls_score_0, self.cls_score_1]
        self.bbox_preds = [self.bbox_pred_0, self.bbox_pred_1]
    ...
    def forward(self, x):
        ...
        # scores = [self.cls_score_0(x), self.cls_score_1(x)]
        # proposal_deltas = [self.bbox_pred_0(x), self.bbox_pred_1(x)]
...
class CosineSimOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # prediction layer for num_classes foreground classes and one background class (hence + 1)
        # Add!!!!!!!!!!!!!!!!!!
        self.cls_score_0 = Linear(input_size, num_classes + 1, bias=False)
        self.cls_score_1 = Linear(input_size, num_classes + 1, bias=False)
        self.scale = scale
        if self.scale == -1:
    ...
    def forward(self, x):
        ...
        # # normalize weight 0
        # temp_norm_0 = (
        #     torch.norm(self.cls_score_0.weight.data, p=2, dim=1)
        #     .unsqueeze(1)
        #     .expand_as(self.cls_score_0.weight.data)
        # )
        # self.cls_score_0.weight.data = self.cls_score_0.weight.data.div(
        #     temp_norm_0 + 1e-5
        # )
        # cos_dist_0 = self.cls_score_0(x_normalized)
        # scores.append(self.scale * cos_dist_0)
        # proposal_deltas.append(self.bbox_pred_0(x))
        
        # # normalize weight 1
        # temp_norm_1 = (
        #     torch.norm(self.cls_score_1.weight.data, p=2, dim=1)
        #     .unsqueeze(1)
        #     .expand_as(self.cls_score_1.weight.data)
        # )
        # self.cls_score_1.weight.data = self.cls_score_1.weight.data.div(
        #     temp_norm_1 + 1e-5
        # )
        # cos_dist_1 = self.cls_score_1(x_normalized)
        # scores.append(self.scale * cos_dist_1)
        # proposal_deltas.append(self.bbox_pred_1(x))
