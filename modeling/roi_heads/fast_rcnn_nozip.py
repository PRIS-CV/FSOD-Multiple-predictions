...
class FastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            scores.append(self.cls_scores[i](x))
            proposal_deltas.append(self.bbox_preds[i](x))
...
class CosineSimOutputLayers(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            temp_norm = (
                torch.norm(self.cls_scores[i].weight.data, p=2, dim=1)
                .unsqueeze(1)
                .expand_as(self.cls_scores[i].weight.data)
            )
            self.cls_scores[i].weight.data = self.cls_scores[i].weight.data.div(
                temp_norm + 1e-5
            )
            cos_dist = self.cls_scores[i](x_normalized)
            scores.append(self.scale * cos_dist)
            proposal_deltas.append(self.bbox_preds[i](x))
